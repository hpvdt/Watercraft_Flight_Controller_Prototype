...
bool readSensor(uint8_t address, float &reading) {
    ...
    *reading = (float)rawData / 100.0; // Example conversion
    return true;
}
...
void calculatePerpendicularBisector(float endPoint[2]) {
    ...
    endPoint[0] = h / 2 * cosf(theta) - h / 2 * sinf(theta);
    endPoint[1] = (b - a) / 2 + h / 2 * cosf(theta);
}
