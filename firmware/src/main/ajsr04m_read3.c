...
void init_sensors() {
    ...
    //Configure Sensor 3
    gpio_set_direction(TRIGGER_3, GPIO_MODE_OUTPUT);
    gpio_set_direction(ECHO_3, GPIO_MODE_INPUT);
}
...
void sensor_task(void *arg) {
    while (1) {
        ...
        float distance3 = measure_distance_cm(TRIGGER_3, ECHO_3);
        ESP_LOGI(TAG, "D1: %.2f cm, D2: %.2f cm, D3: %.2f cm", distance1, distance2, distance3);
